import math, numpy as np, random, matplotlib.pylab as plt, matplotlib.path as mpath, TSPRead as FileRead, time
...
N = 0
inf = float("inf")
xs = FileRead.xs #x coordinates of the nodes
ys = FileRead.ys #y coordinates of the nodes
ns = FileRead.nodes #Node numbers
...
themax = cities.max()
cities = cities/themax #Normalizing the coordinates with respect to the maximum-value coordinate
...
for i in range(2*N): #Number of SOM Nodes: 2*N: Random
    ...
    nodetoadd = Node(x/themax,y/themax) #Creation of a SOM node, with its coordinates normalized
    if len(somnodes) > 0:
        prev = somnodes[-1]
        nodetoadd.set_prev(prev) #Connect the last known SOM node as the previous of the new one
        prev.set_next(nodetoadd) #Connect the new node as the next of the last known SOM node 
    somnodes.append(nodetoadd) #Add the node to the list of SOM nodes
    
#Connect the first and the last nodes of the SOM nodes together
first = somnodes[0]
last = somnodes[-1]
first.set_prev(last)
last.set_next(first)
...
#plot(somtodata(w),30) #Plot the initial positions of the SOM nodes over the whole data
for i in range(NumIters):
    ...
    for index in indices:
        ...
        if i == 0: #If it is the first iteration
            ...
            BMU = prev_BMU
            for j in range(1,BMU_effect): #Pulls BMU's "forward" neighborhood along with BMU
            ...
            for j in range(1,BMU_effect): #Pulls BMU's "backward" neighborhood along with BMU
...
route = [int(i) for i in route] #While path stores the coordinates of the nodes, route stores the node numbers
...
totalDistance = totaldist*themax
print("Total distance of the path is %.10f"%totalDistance) #Denormalize the total distance
if isinstance(FileRead.opttour,basestring) == 0:
    optcost = FileRead.optcost
    print("Optimal tour cost is %.10f"%optcost)
